...
# 注：官方源码给了两种情况，一种是用Vit，另一种是用ResNet。
...
# 注意力块，包括了layernorm,attention,layernorm,mlp
...
# 该类为一个包装结构，重点是重复调用ResidualAttentionBlock结构
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        scale = width ** -0.5
        # 随机初始化位置编码和CLS token，并转变成可以学习的参数。
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        # 注意+1是预留了cls token的位置
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))
        self.ln_pre = LayerNorm(width)
        # Parameter 是将tensor转换为可以学习的参数
        self.transformer = Transformer(width, layers, heads)

        self.ln_post = LayerNorm(width)
        # proj特殊结构，该结构若使用将进一步将图像特征表达进行变换，该变换的self.proj是可学习参数
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor):
        # x=[1,3,224,224]
        x = self.conv1(x)  # shape = [*, width, grid, grid] # 将图片分成[16,16]个patch [1,768,14,14]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2],合并高宽 [1,768,196]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] ，更换位置 [1,196,768]
        # cat拼接函数，to强制类型转换
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width],添加cls token[1,197,768]
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            vision_heads = vision_width // 64    # 768/64 = 12  //是向下整除
        ...
        # self.text_projection特殊结构，该结构若使用将进一步将文本特征表达进行变换，该变换的self.text_projection是可学习参数
        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim))
        # 其中logit_scale是一个实数值，用于调整图像特征和文本特征之间点积（即相似度矩阵）的范围。
        # 代码中是nn.Parameter(torch.ones([]) * np.log(1 / 0.07))，是一个可学习的参数。模型可以在训练过程中自动调整这个缩放因子。
    ...
    def forward(self, image, text):
        ...
        # .t()是转置
        logits_per_text = logits_per_image.t()  # 变成文本特征 未转置是图像特征
