...
def make_batch(sentences):
    input_batch = [[src_vocab[n] for n in sentences[0].split()]]  # 取sentences第一个数据中的元素,输出是[[1,2,3,4,0]]
    output_batch = [[tgt_vocab[n] for n in sentences[1].split()]]
    target_batch = [[tgt_vocab[n] for n in sentences[1].split()]]
    return torch.LongTensor(input_batch), torch.LongTensor(output_batch), torch.LongTensor(
        target_batch)  # input_batch 返回为： tensor([[1, 2, 3, 4, 0]])


# 为 mask Multi-head attention 生成掩码矩阵表示能看见什么，即把上三角设置为不能看见的
def get_attn_subsequent_mask(seq):
    ...
    subsequence_mask = torch.from_numpy(subsequence_mask).byte()  # 转变为tensor类型
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        scores = torch.matmul(Q, K.transpose(-1, -2)) / np.sqrt(d_k)  # 矩阵乘法
        ## 然后关键词地方来了，attn_mask，把被mask的地方置为无限小，softmax之后基本就是0，对q的单词不起作用
        scores.masked_fill_(attn_mask, -1e9)  # Fills elements of self tensor with value where mask is one.
        attn = nn.Softmax(dim=-1)(scores)  # 注意dim=-1 在第三维进行softmax
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # 然后计算ScaledDotProductAttention，有两个结果：context: [batch_size x n_heads x len_q x d_v], attn: [batch_size x n_heads x len_q x len_k]
...
## PositionalEncoding 位置编码的实现：
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, droput=0.1, max_len=5000):
        ...
        pe = torch.zeros(max_len, d_model)  # 初始化一个大小为 max_len X d_model 的全零矩阵tensor类型
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)  # unsqueeze（1）是升维变成 max_len X 1 的一个矩阵
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_term)  # pe[:, 0::2]这个用法，就是从0开始到最后面，补长为2，其实代表的就是偶数位置
        pe[:, 1::2] = torch.cos(position * div_term)  # pe[:, 1::2]这个用法，就是从1开始到最后面，补长为2，其实代表的就是奇数位置
        ## pe:[max_len*d_model]

        pe = pe.unsqueeze(0).transpose(0, 1)
        ## pe:[max_len*1*model]

        self.register_buffer('pe', pe)  ##定义一个缓冲区，简单理解就是保持参数不更新。
...
class EncoderLayer(nn.Module):
    ...
    def forward(self,enc_inputs,enc_self_attn_mask):
        ## 下面这个就是做自注意力层，输入是enc_inputs，形状是[batch_size x seq_len_q x d_model] 需要注意的是最初始的QKV矩阵是等同于这个输入的，去看一下enc_self_attn函数
...
class Decoder(nn.Module):
    ...
    def forward(self,dec_inputs,enc_inputs,enc_outputs): # dec_inputs : [batch_size x target_len]
        ...
        # get_attn_pad_mask 自注意力时pad部分
        dec_self_attn_pad_mask = get_attn_pad_mask(dec_inputs,dec_inputs)

        #get_attn_subsequent_mask 自注意力的mask部分，就是当前单词之后看不到，使用一个上三角为1的矩阵
...
if __name__ == '__main__':
    ...
    n_heads = 8  # numer of heads in Multi-Head Attention
    model = Transformer()
