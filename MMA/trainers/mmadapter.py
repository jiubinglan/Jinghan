...
class AdapterLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
# 计算类别数量，检查输入图像尺寸是否与CLIP模型要求的尺寸一致。然后调用_build_text_embedding方法构建文本嵌入。
...
# 首先构建文本adapter，
        ...
        self.text_adapter_func = lambda x: self.return_text_adapter(index=x) # 定义了一个text_adapter_func的lamda函数，该函数接受一个x，并将其作为索引传递给return_text_adapter方法
        self.text_adapter = self._build_adapter(
            clip_model.ln_final.weight.shape[0],  # CLIP最后一层线性层权重的大小
            len(clip_model.transformer.resblocks), # CLIP中Transformer模块中残差块的数量
...
# 构建共享adapter
    ...
    def _build_text_embedding(self, cfg, classnames, clip_model):
        ...
        with torch.no_grad():
            embedding = clip_model.token_embedding(tokenized_prompts).type(dtype) # 使用CLIP模型中的token_embedding方法将变标记化的提示转换为嵌入向量，并转为CLIP模型指定的数据类型
# 定义缓存区，以便训练过程中保持其值不变
    ...
    def _build_adapter(self, d_model, n_layers, l_start, l_end, mid_dim, dtype):
        ...
        adapter = [None] * (n_layers + 1) # 创建一个长度为（n+1）的列表，来存储adapter的各个层
        ...
        adapter = nn.ModuleList([a for a in adapter]) # 将适配器列表转换为nn.ModuleList，以便在PyTorch中进行模型训练。
        for m in adapter.modules():
            if isinstance(m, nn.Linear): # 如果模块是线性层
                nn.init.kaiming_normal_(m.weight, mode="fan_out", nonlinearity="relu") #使用Kaiming初始化方法初始化权重。
                nn.init.constant_(m.bias, 0) # 将偏置项初始化为0。

        if dtype == torch.float16: # 如果数据类型为半精度浮点数（float16），则将所有模块转换为半精度浮点数。
            for m in adapter.modules():
                m.half()
